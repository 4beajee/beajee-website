  ...
  return (
    <div className="min-h-dvh bg-transparent overflow-x-hidden">
      ...
      >
        ...
        <CodexIcon className="hidden sm:block pointer-events-none absolute left-4 top-[18%] w-12 h-12 select-none animate-agent-codex sm:left-[10%] sm:top-[20%] sm:w-16 sm:h-16 md:left-[12%]" />
        <OpenClawIcon className="hidden sm:block pointer-events-none absolute right-4 top-[14%] w-12 h-12 select-none animate-agent-openclaw sm:right-[8%] sm:top-[16%] sm:w-16 sm:h-16 md:right-[10%]" />
        <FacebookIcon className="hidden sm:block pointer-events-none absolute left-5 top-[46%] w-12 h-12 select-none animate-agent-facebook sm:left-[11%] sm:top-[48%] sm:w-16 sm:h-16 md:left-[13%]" />
        <KimiIcon className="hidden sm:block pointer-events-none absolute right-5 top-[46%] w-12 h-12 select-none animate-agent-kimi sm:right-[11%] sm:top-[48%] sm:w-16 sm:h-16 md:right-[13%]" />

        <div className="hidden sm:flex pointer-events-none absolute left-6 bottom-[20%] flex h-12 w-12 items-center justify-center rounded-full bg-white p-2 text-black shadow-sm select-none animate-agent-hermes sm:left-[12%] sm:bottom-[22%] sm:h-16 sm:w-16 sm:p-2.5 md:left-[14%] overflow-hidden">
          <HermesIcon className="h-full w-full" />
        </div>

        <ClaudeCodeIcon className="hidden sm:block pointer-events-none absolute right-6 bottom-[16%] w-12 h-12 select-none animate-agent-claude sm:right-[14%] sm:bottom-[18%] sm:w-16 sm:h-16 text-white md:right-[16%]" />
        ...
        <div className="relative mt-7 w-full max-w-2xl text-center">
          ...
          <div className="hero-cta mt-12 flex flex-col items-center gap-3 sm:hidden">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-neutral-500">
              {t("landing.supportedAgents.title")}
            </p>
            <div className="flex flex-wrap items-center justify-center gap-5 py-3 px-5 rounded-2xl bg-white/[0.02] border border-white/[0.05]">
              <CodexIcon className="h-8 w-8 text-white select-none" />
              <OpenClawIcon className="h-8 w-8 select-none" />
              <FacebookIcon className="h-8 w-8 select-none" />
              <KimiIcon className="h-8 w-8 select-none" />
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white p-1.5 text-black shadow-sm select-none overflow-hidden">
                <HermesIcon className="h-full w-full" />
              </div>
              <ClaudeCodeIcon className="h-8 w-8 text-white select-none" />
            </div>
          </div>
        </div>
      </section>
