...
export function CookieConsent() {
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto rounded-xl max-sm:rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a]/95 backdrop-blur-xl p-3 sm:px-5 sm:py-4 shadow-[0_-4px_40px_rgba(0,0,0,0.5)] max-h-[90vh] overflow-y-auto">
        {!showCustomize ? (
          /* ── Default banner view ── */
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
            ...
            <div className="grid grid-cols-3 sm:flex sm:items-center gap-2 sm:shrink-0">
              <button
                ...
                className={cx(subtleButtonSmallClass, "border-0 bg-transparent text-neutral-400 ring-white/[0.10] max-sm:!rounded-full")}
              >
                {t("decline")}
              </button>
              <button
                onClick={() => setShowCustomize(true)}
                className={cx(subtleButtonSmallClass, "border-0 bg-transparent ring-white/[0.10] max-sm:!rounded-full")}
              >
                {t("customize")}
              </button>
              <button
                onClick={() => dismiss("accepted", ALL_ACCEPTED)}
                className={cx(primaryButtonSmallClass, "max-sm:!rounded-full")}
              >
                {t("accept")}
              </button>
            </div>
